...
export async function POST(request: Request) {
  try {
    ...
    for (const reserva of historicoCliente) {
      if (reserva.status.includes('PAGO') || reserva.status.includes('SINAL') || reserva.status === 'CONFIRMADO') {
        return NextResponse.json({ 
          ...
          O sistema não permite agendar dois serviços simultaneamente.
          
          ⚠️ Para trocar de serviço ou mudar o horário, entre em contato via WhatsApp.` 
        }, { status: 409 });
      }
    ...
    for (const vaga of vagaOcupada) {
      ...
      // Bloqueio Temporário (Regra de 2 minutos)
