...
export default function AdminDashboard() {
  ...
  const slug = params?.slug as string; // <--- O SEGREDO: Capturamos o slug da URL
  
  const [config, setConfig] = useState<any>({ porcentagemSinal: 50, nomeEstabelecimento: "" });
  ...
  // Funções de parsing de data (mantidas originais)
  ...
  useEffect(() => {
    ...
    // --- MUDANÇA: Busca configuração da loja específica (/api/settings) ---
    fetch(`/api/settings?slug=${slug}`)
      .then((res) => res.json())
      .then((data) => {
        if (data && !data.error) {
            // Mapeia os dados do banco novo para o formato que o componente espera
            setConfig((prev: any) => ({ 
                ...prev, 
                ...data,
                // Garante compatibilidade de nomes se necessário
                nomeEstabelecimento: data.nomeEstabelecimento || data.nome
            }));
        }
      })
  ...
  const handleLogout = async () => {
    // await fetch("/api/auth/logout", { method: "POST" }); // Comentei logout de sessão por enquanto
    router.push("/admin/login");
  };

  const handleConfirmPayment = async (id: string) => {
    const porcentagemRestante = 100 - (config.porcentagemSinal || 50);
  ...
  // --- LÓGICA DE FILTROS (Mantida Idêntica) ---
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 p-4 md:p-8 overflow-x-hidden relative flex flex-col">
      ...
      <header className="relative z-10 flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-zinc-900/50 backdrop-blur-xl p-4 md:p-6 rounded-3xl border border-white/5">
        <div className="text-center md:text-left">
          <h1 className="text-xl md:text-2xl font-black tracking-tight text-white flex items-center justify-center md:justify-start gap-3">
            ...
            {config.nomeEstabelecimento || "Painel Admin"} 
          </h1>
                          ...
                          <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  {(() => {
                                      ...
                                      if (pago > 0 && pago <= 0.50) {
                                          return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border border-yellow-500/20 bg-yellow-500/10 text-yellow-500">Reserva ({config.porcentagemSinal}%)</span>;
                                      } 
                                      if (pago >= 1.0) {
                                          return <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-black text-emerald-500 uppercase tracking-tighter">Integral</span>;
                                      }
                                      return null;
                                  })()}
                                </div>
                                            ...
                                            if (isPaid(booking.status) && pago > 0 && pago < 1.00) { 
                                                return (
                                                    <div className="flex flex-col gap-2 mt-2 pt-2 border-t border-white/5">
                                                            ...
                                                            <button onClick={() => handleConfirmPayment(booking.id)}
      ...
      <footer className="relative z-10 w-full mt-20 pb-8 flex flex-col items-center justify-center gap-4 opacity-70 border-t border-white/5 pt-8">
        <p className="text-zinc-400 font-bold tracking-widest text-xs uppercase text-center">© {new Date().getFullYear()} {config.nomeEstabelecimento || "Barbearia"}</p>
