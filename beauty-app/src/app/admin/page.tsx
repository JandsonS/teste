...
interface Booking {
  ...
  paymentType: string; // 'FULL' ou 'DEPOSIT'
  priceTotal: number;
  pricePaid: number;
  pricePending: number;
  createdAt: string;
...
export default function AdminDashboard() {
  ...
  // Função para gerar link do WhatsApp com mensagem de confirmação
  const getWhatsAppLink = (phone: string, name: string, date: string, time: string, service: string) => {
    const cleanPhone = phone.replace(/\D/g, '');
    const message = `Olá ${name}, tudo bem? Passando para confirmar seu agendamento de *${service}* para o dia *${format(parseISO(date), "dd/MM", { locale: ptBR })}* às *${time}*.`;
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 p-4 md:p-8 overflow-x-hidden relative">
      ...
      <header className="relative z-10 flex flex-col md:flex-row justify-between items-center mb-10 gap-4 bg-zinc-900/50 backdrop-blur-xl p-6 rounded-3xl border border-white/5">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Gerencie seus agendamentos em tempo real.</p>
        </div>
      ...
      <main className="relative z-10 max-w-6xl mx-auto">
        ...
        {bookings.length === 0 ? (
          <div className="text-center py-20 bg-zinc-900/30 rounded-3xl border border-white/5 backdrop-blur-sm">
            ...
            <p className="text-zinc-600 text-sm">Aguarde novos clientes agendarem.</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {bookings.map((booking, index) => (
              ...
              >
                {/* Status Bar Colorida (Único lugar com cor forte para facilitar leitura) */}
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="flex items-center gap-3 text-zinc-300 bg-white/5 p-3 rounded-xl border border-white/5">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-sm">{format(parseISO(booking.bookingDate), "dd/MM/yyyy")}</span>
                        </div>
                    ...
                    <div className="bg-zinc-900 p-4 rounded-xl text-sm border border-zinc-800 flex justify-between items-center">
                        ...
                        {booking.pricePending > 0 ? (
                            <div className="text-right">
                                <p className="text-zinc-500 text-[10px] uppercase font-bold">Falta Pagar</p>
                                <p className="text-white font-bold mt-0.5 border-b border-amber-500/50 pb-0.5">{formatCurrency(booking.pricePending)}</p>
                            </div>
                        ) : (
                             <div className="text-right">
                                <p className="text-emerald-500 text-[10px] uppercase font-bold flex items-center gap-1 justify-end"><CheckCircle2 size={10}/> Total Pago</p>
                            </div>
                        )}
                    </div>
